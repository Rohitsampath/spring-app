...
public class ActorOperationController {
	...
	public String showReport(
			@PageableDefault(page = 0, size = 3, sort = "category", direction = Sort.Direction.ASC) Pageable pageable,
	...
	/*
	 * @PostMapping("/register") // reload page data will be added public String
	 * registerActor(@ModelAttribute("actor") Actor actor,Map<String, Object>map) {
	 * 
	 * Iterable<Actor>list=actorService.showAllActors(); //use service String
	 * msg=actorService.registerActors(actor);
	 * 
	 * //keep result is model atttr map.put("resultMsg", msg); map.put("listActors",
	 * list);
	 * 
	...
	/*
	 * @PostMapping("/register") //solution 1 implementing PRG pattern public String
	 * registerActor(@ModelAttribute("actor") Actor actor,RedirectAttributes attr) {
	 * 
	 * //use service String msg=actorService.registerActors(actor);
	 * 
	 * //keep result is model atttr attr.addFlashAttribute("resultMsg", msg);
	 * 
	 * System.out.println("ActorOperationController.registerActor()");
	 * 
